...
void TIM6_init(void)
{
  ...
  TIM_TimeBaseInitStructure.TIM_Period = 250 - 1; 														//自动重装载值	即CNT加到500就置零
	TIM_TimeBaseInitStructure.TIM_Prescaler= 9000 - 1;  												//定时器分频		即90M/9000hz使CNT+1	所以这样子计算出来每次进中断的时间为
	TIM_TimeBaseInitStructure.TIM_CounterMode=TIM_CounterMode_Up; 							//向上计数模式												 250/(90M/9000)S		即0.025s 25ms
...
void TIM7_init(void)
{
  ...
  TIM_TimeBaseInitStructure.TIM_Period = 500 - 1; 														//自动重装载值	即CNT加到500就置零
	TIM_TimeBaseInitStructure.TIM_Prescaler= 9000 - 1;  												//定时器分频		即90M/9000hz使CNT+1		所以这样子计算出来每次进中断的时间为
	TIM_TimeBaseInitStructure.TIM_CounterMode=TIM_CounterMode_Up; 							//向上计数模式												  500/(90M/9000)S		即	0.05s 50ms
	TIM_TimeBaseInitStructure.TIM_ClockDivision=TIM_CKD_DIV1; 
